...
if (!function_exists('info_general')) {
...
}if (!function_exists('getPaymentMethodTranslation')) {

		function getPaymentMethodTranslation($lang, $id) {
		  if(!empty($id)){
          $CI = get_instance();
          $CI->load->model('Admin/Payment_methods_model');
          $res = $CI->Payment_methods_model->getPaymentMethodTranslation($lang,$id);
          return $res;
		  }else{
            return '';
		  }
...
}if (!function_exists('getTicketLevelTranslation')) {

		function getTicketLevelTranslation($lang, $id) {
		  if(!empty($id)){
          $CI = get_instance();
          $CI->load->model('Admin/Ticket_level_model');
          $res = $CI->Ticket_level_model->getTicketLevelTranslation($lang,$id);
          return $res;
		  }else{
            return '';
		  }
...
}if (!function_exists('getBaggageTranslation')) {

		function getBaggageTranslation($lang, $id) {
		  if(!empty($id)){
          $CI = get_instance();
          $CI->load->model('Admin/Baggages_model');
          $res = $CI->Baggages_model->getBaggageTranslation($lang,$id);
          return $res;
		  }else{
            return '';
		  }
	...
	function curlCall($url, $postfields = null, $curlopts = array()) {
		...
		$response->list = $updatesList->list;
    ...
    function checkUrlParams($method, $params = array(),$isValidLang) {
    	$CI = get_instance();
    	$urlBase = $method;
