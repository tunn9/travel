...
class Offers extends MX_Controller {
		...
		function __construct() {
				...
				$this->data['ckconfig']['language'] = 'vi';
		...
		function index() {
        		...
        		$xcrud->button("javascript: delfunc('{offer_id}','$delurl')",'Xóa','fa fa-times','btn-danger',array('target'=>'_self','id' => '{offer_id}'));
                ...
                $xcrud->label('offer_thumb','Ảnh');
                $xcrud->label('offer_forever','Bộ sưu tập');
                $xcrud->label('offer_order','Thứ tự');
                $xcrud->label('offer_price','Giá');
                $xcrud->label('offer_status','Trạng thái');
                $xcrud->label('offer_title','Tiêu đề');
				...
				$this->data['page_title'] = 'Quản lí đề xuất';
				$this->data['main_content'] = 'temp_view';
				$this->data['header_title'] = 'Quản lí đề xuất';
        ...
        $this->load->model('Admin/Uploads_model');
				...
				if (!empty ($addoffer)) {
						$this->form_validation->set_rules('offertitle', 'Tiêu đề đề xuất', 'trim|required');
						$this->form_validation->set_rules('offerdesc', 'Mô tả đề xuất', 'trim|required');
				 		if ($this->form_validation->run() == FALSE) {
								...
								$this->session->set_flashdata('flashmsgs', 'Thêm đề xuất thành công!');
								echo "done";
						}
				}
				else {

				$this->data['page_title'] = 'Thêm đề xuất';
				...
				if (!empty ($updateoffer)) {
						...
						$this->form_validation->set_rules('offerdesc', 'Mô tả đề xuất', 'trim|required');
				 		if ($this->form_validation->run() == FALSE) {
								...
								$this->session->set_flashdata('flashmsgs', 'Cập nhật đề xuất thành công1');
								echo "done";
						}
				}
				else {
		...
		function settings() {
				...
				$this->data['page_title'] = 'Cài đặt đề xuất đặc biệt';
				$this->load->view('template', $this->data);
		}
        ...
        function gallery($id) {
				...
				$this->data['page_title'] = 'Bộ sưu tập đề xuất';
