...
//process booking
		function processBookingguest() {
				...
				$this->form_validation->set_rules('email', trans("094"), 'trim|required|valid_email|is_unique[pt_accounts.accounts_email]');
...
<div class='alert alert-danger'>" . trans("0313") . "</div>";
		...
		public function update_payment_method_order() {
			$postid = $this->input->post('id');
			$order = $this->input->post('order');
			 $this->load->model('Admin/Payment_methods_model');
			$this->db->select('pmethod_id');
			$total = $this->db->get('pt_payment_methods')->num_rows();
  
			if($order > $total){
			  echo '0';
			}else{
			$this->Payment_methods_model->update_payment_method_order($postid, $order);
			  echo '1';
			}
    ...
    // Delete CheapFlight
    function delCheapFlight(){
        $this->load->model('Admin/Cheap_flight_model');
        $id = $this->input->post('id');
        $this->Cheap_flight_model->deleteCheapFlight($id);
    }

    // Delete Multiple CheapFlights
    function delMultipleCheapFlight(){
        $this->load->model('Admin/Cheap_flight_model');
        $items = $this->input->post('items');

        foreach($items as $item){
            $this->Cheap_flight_model->deleteCheapFlight($item);
        }
       ...
       function deleteMultipleOfferImages(){
		...
		function delPaymentMethod(){
			$this->load->model('Admin/Payment_methods_model');
			$id = $this->input->post('id');
			$this->Payment_methods_model->delete_payment_method($id);
		}

		// delete multiple payment methods
		function delMultiplePaymentMethods(){
			$this->load->model('Admin/Payment_methods_model');
			$items = $this->input->post('items');
			foreach($items as $item){
				$this->Payment_methods_model->delete_payment_method($item);
			}
		}

		// delete group airline
		function delGroupAir(){
			$this->load->model('Admin/Group_airline_model');
			$id = $this->input->post('id');
			$this->Group_airline_model->delete_group_airline($id);
		}

		// delete multiple group airlines
		function delMultipleGroupAirs(){
			$this->load->model('Admin/Group_airline_model');
			$items = $this->input->post('items');
			foreach($items as $item){
				$this->Group_airline_model->deleteAirline($item);
			}
		}
		...
		function delMultipleAirlines(){
			$this->load->model('Admin/Airlines_model');
			$items = $this->input->post('items');
			foreach($items as $item){
				$this->Airlines_model->delete_airline($item);
			}
		}

		// delete ticket level
		function delTicketLevel(){
			$this->load->model('Admin/Ticket_level_model');
			$id = $this->input->post('id');
			$this->Ticket_level_model->delete_ticket_level($id);
		}

		// delete multiple ticket levels
		function delMultipleTicketLevels(){
			$this->load->model('Admin/Ticket_level_model');
			$items = $this->input->post('items');
			foreach($items as $item){
				$this->Ticket_level_model->delete_ticket_level($item);
			}
		}
		...
		function delBaggage(){
			...
			$id = $this->input->post('id');
			$this->Baggages_model->delete_baggage($id);
		}

		// delete multiple baggages
		function delMultipleBaggages(){
			$this->load->model('Admin/Baggages_model');
			$items = $this->input->post('items');
			foreach($items as $item){
				$this->Baggages_model->delete_baggage($item);
			}
		}
