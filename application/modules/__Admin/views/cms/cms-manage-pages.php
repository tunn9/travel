<script>
  $(function(){
    ...
    $('.del_selected').click(function(){
      ...
      if(count_checked == 0) {
      $.alert.open('info', 'Hãy chọn trang để xóa.');
        return false;
         }
  ...
  $.post("<?php echo base_url();?>admin/ajaxcalls/delete_multiple_pages", { pagelist: pages }, function(theResponse){
      ...
      var count_checked = $("[name='page_ids[]']:checked").length;
      if(count_checked == 0) {
         $.alert.open('info', 'Chọn trang để vô hiệu hóa');
        return false;
         }


           $.alert.open('confirm', 'Bạn có chắc chắn muốn vô hiệu hóa?', function(answer) {
    if (answer == 'yes')
  ...
  $.post("<?php echo base_url();?>admin/ajaxcalls/disable_multiple_pages", { pagelist: pages }, function(theResponse){
    ...
    // Enable multiple pages
      ...
      if(count_checked == 0) {
        ...
        $.alert.open('info', 'Hãy chọn trang để Bật');
       return false;
         }


         $.alert.open('confirm', 'Bạn có chắc chắn muốn Bật?', function(answer) {
    if (answer == 'yes')
...
<div class="<?php echo body;?>">
  ...
  <div class="panel panel-primary table-bg">
    <div class="panel-heading">
      <span class="panel-title pull-left"><i class="fa fa-list-alt"></i> Quản lí bố cục trang</span>
      ...
      <table cellpadding="0" cellspacing="0" border="0" class="datatable table table-hover table-striped table-bordered">
        ...
        <tbody>
          ...
          <tr class="">
            ...
            <td>
              <span class="dropdown">
                <a href="#" class="dropdown-toggle" data-toggle="dropdown"><span class="fa fa-globe"></span> <?php echo trans('0229'); ?> <b class="caret"></b></a>
