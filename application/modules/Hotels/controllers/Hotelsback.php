...
class hotelsback extends MX_Controller {
		...
		function index() {
                ...
                $xcrud->button("javascript: delfunc('{hotel_id}','$delurl')",'DELETE','fa fa-times', 'btn-danger',array('target'=>'_self','id' => '{hotel_id}'));
                }
                $xcrud->limit(50);
				$xcrud->unset_add();
				$xcrud->unset_edit();
...
if(!$this->data['addpermission'] && !$this->editpermission && !$this->deletepermission){
		...
		$output .= '<input type="hidden" name="ids_list" value="'.$ids_list.'">';
					   ...
					   //	 $datefmt = $this->input->post('dateformat');
						...
						$xcrud->join('room_hotel', 'pt_hotels', 'hotel_id');
						$xcrud->join('room_type', 'pt_hotels_types_settings', 'sett_id');
...
//	$xcrud->unset_add();
                		...
                		$xcrud->multiDelUrl = base_url().'admin/hotelajaxcalls/delMultipleRooms';
                        $this->data['content'] = $xcrud->render();
