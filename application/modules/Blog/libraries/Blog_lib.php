...
//set car id by id
		...
		function latestPostsHomepage() {
			...
			foreach($posts as $p){
				...
				$results->posts[] = (object)array('id' => $p->post_id, 'date' => $p->post_created_at, 'title' => $this->title,'thumbnail' => $this->thumbnail,'desc' => $this->desc, 'shortDesc' =>  character_limiter($shortdesc,60),'slug' => base_url().'blog/'.$this->slug);
			}
			return $results;
		}
