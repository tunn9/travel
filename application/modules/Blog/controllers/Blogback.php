...
class Blogback extends MX_Controller {
	...
	function index() {
		...
		$xcrud->label('post_title','Tên bài viết')->label('pt_blog_categories.cat_name','Danh mục')->label('post_created_at','Ngày tạo')->label('post_order','Thứ tự')->label('post_status','Trạng thái')->label('post_img','Ảnh');
		...
		if($this->editpermission){
			$xcrud->button(base_url() . $this->data['adminsegment'] . '/blog/manage/{post_slug}', 'Cập nhật', 'fa fa-edit', 'btn btn-warning', array('target' => '_self'));
		...
		if($this->deletepermission){
			...
			$xcrud->button("javascript: delfunc('{post_id}','$delurl')",'Xóa','fa fa-times', 'btn-danger',array('target'=>'_self'));
        }
		...
		$xcrud->column_callback('post_status', 'create_status_icon');
	...
	function category() {
		...
		$xcrud->columns('cat_name,cat_status');
		$xcrud->label('cat_name','Tên danh mục')->label('cat_status','Trạng thái hoạt động');
		...
		$xcrud->column_callback('cat_status', 'create_status_icon');
